...
export default function ProductCard({ id, imageUrl, title, brand, model, price, size, tags, status = "AVAILABLE" }: ProductCardProps) {
    const isReserved = status === "RESERVED";
    const isSold = status === "SOLD";
    const displayTitle = brand ?? title ?? '';
    const displaySub = model ?? '';
        ...
        <Link href={`/product/${id}`} className={`group block w-full mb-6 relative ${isReserved || isSold ? 'pointer-events-none' : ''}`}>
            ...
            <div className="mt-2 flex justify-between items-start">
                <div className="flex flex-col">
                    <span className="font-sans font-black uppercase text-sm leading-tight tracking-wide">{displayTitle}</span>
                    {displaySub && <span className="text-foreground/70 text-sm font-medium">{displaySub}</span>}
                </div>
